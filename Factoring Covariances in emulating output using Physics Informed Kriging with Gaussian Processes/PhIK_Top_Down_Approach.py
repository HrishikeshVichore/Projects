''' Yet to be updated and completed'''
print('Yet to be updated and completed')
exit()
...
class PhIK_1D():
    ...
    def fit(self):
        print(f'Number of space points = {self.space_points.shape}')
        print(f'Number of param points = {self.param_points.shape}')
        ...
        Cov_matrix_space = np.array([self.k(i,self.mew_space_poins,i,self.mew_space_poins) for i in self.space_points[0,:]])
        noise = np.random.normal(np.mean(Cov_matrix_space),np.std(Cov_matrix_space),Cov_matrix_space.shape)
        Cov_matrix_space += noise 
        print(f'Spatial covariance shape = {Cov_matrix_space.shape}')
        ...
        print(f'Parameter covariance matrix shape = {Cov_matrix_param.shape}')
        '''
        ...
        print(f'Kronecker Product shape = {self.R_inv.shape}')
    ...
    def predict(self,x_star, q_star):
        ...
        corr_matrix = np.array([[j*self.k(i,self.mew_param_points, q_star,new_mean) for i in self.param_points]for j in self.space_points]).T
        print(f'corr_matrix shape = {corr_matrix.shape}')
        
        R = np.array([np.squeeze(i.T@j) for i,j in zip(corr_matrix, self.R_inv)])
        # print(R.shape)
        
        y_cap = self.h_beta + R@zmg
        print(f'y\u0302(x*,q*) shape = {y_cap.shape}') 
        # print(y_cap)
...
P.predict(x_star = 0.3, q_star = 0.5)

"""
Before
H_beta = scalar
x* -> vector 13 space pts with 12 parameter pts. (proposed change)
h_beta -> vector (13,1)
y -> original o/p (13,12)

y-h_beta -> matrix - scalar
after change
y-h_beta -> matrix - vector
(13,12) - (13,1)

for each pt in vector:
run all of the code
o/p -> matrix for each pt in vector
13 space pts. 13 matrices.
"""
