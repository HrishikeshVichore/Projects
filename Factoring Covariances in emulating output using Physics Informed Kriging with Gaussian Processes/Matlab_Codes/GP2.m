function [GPR] = GP2(X,display_graph)
...
if display_graph
    ...
    fplot(@(x) x.*cos(x),[0,10],'--r')   % Function plot of x*sin(x)
    plot(x,ypred,'g')                   % GPR predictions
    patch([x;flipud(x)],[yint(:,1);flipud(yint(:,2))],'k','FaceAlpha',0.1); % Prediction intervals
    hold off
    title('GPR Fit of g(x)')
    legend({'Noisy observations','g(x) = x*cos(x)','GPR predictions','95% prediction intervals'},'Location','best')
end