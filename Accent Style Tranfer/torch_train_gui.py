...
from utils import *
from torch_model import RandomCNN as Model
import time
import math
import argparse
from librosa import get_duration as duration, load
import os
from torch.optim.lr_scheduler import CyclicLR as lrs
...
parser.add_argument('-kernel', help='Choose the kernel initialization\n[random],[content],[style]', default = 'Random')
...
kernel = args.kernel
print(f'{kernel} kernel initialized')
kernels = {'Random':Variable(torch.randn(a_content_torch.shape) * 1e-3), 'Content':a_C_var.detach().clone(), 'Style':a_S_var.detach().clone()}
for param in model.parameters():
    param.grad = None

a_G_var = kernels[kernel]
if cuda:
    a_G_var = a_G_var.cuda()
a_G_var.requires_grad = True
lr = learning_rate if learning_rate else 0.001
optimizer = torch.optim.AdamW([a_G_var], lr)
scheduler = 0 if learning_rate else lrs(optimizer,base_lr=lr, max_lr=0.2, cycle_momentum=0)

# coefficient of content and style
style_param = float(args.style_weight)
content_param = float(args.content_weight)
...
try:
    for epoch in range(1, num_epochs + 1):
        ...
        scaler.scale(loss).backward(retain_graph=0)
        ...
        if np.round(loss.item(),2) <= 1.0:
            print("{} {}% {} content_loss:{:4f} style_loss:{:4f} total_loss:{:4f}".format(epoch,
                                                                                          epoch / num_epochs * 100,
                                                                                          timeSince(start),
                                                                                          content_loss.item(),
                                                                                          style_loss.item(), loss.item()))
            input('Minimum loss achieved.\nHit Enter to exit.')
            break
except:
    print("{} {}% {} content_loss:{:4f} style_loss:{:4f} total_loss:{:4f}".format(epoch,
                                                                                          epoch / num_epochs * 100,
...
file_name = f'{file_name}_{content_param}_{style_param}_{learning_rate}_{style_name}_{kernel}'
