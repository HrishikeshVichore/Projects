...
# load only 2 seconds of audio to train on it.
a_content, sr = load(CONTENT_FILENAME, duration=2)
a_style, sr = load(STYLE_FILENAME, duration=2)

if duration(a_style, sr) < duration(a_content, sr):
    a_style = extend_audio(a_style, sr, duration(a_content, sr), save_output=1)
